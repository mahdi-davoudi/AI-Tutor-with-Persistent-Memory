...
ChatService.send_message
  ...
  → save user message
  → load recent history
  → embed the user message                (EmbeddingService)
...
- Embeddings are produced by `EmbeddingService` using `sentence-transformers/all-MiniLM-L6-v2` (384 dimensions, normalized, cosine similarity).
- Vector writes are **best-effort**: a Qdrant failure never rolls back a successful Mongo write.
- If semantic search fails, chat falls back to importance-sorted memories.

Key files:

- `app/services/embedding_service.py` — encode single / batch texts
- `app/core/vector_db.py` — Async Qdrant client + collection bootstrap
- `app/services/memory_service.py` — CRUD + semantic search + vector sync
- `app/services/document_service.py` — ingest + RAG over document chunks
...
| Area            | Examples                                              |
|-----------------|-------------------------------------------------------|
| Users           | register, login, get/update user                      |
| Chat            | `POST /api/v1/chat`                                   |
| Memory          | list / upsert / delete memories                       |
| Profile         | get / generate learning profile                       |
| Quiz            | generate quiz                                         |
| Documents       | upload, list, delete                                  |
| Recommendations | `GET /recommendations/{user_id}` (no `/api/v1` prefix in current router) |
...
| Phase | Capability                                              |
|-------|---------------------------------------------------------|
| 1     | Backend layout, auth, CRUD, exceptions                  |
| 2     | Memory extraction + persistence                         |
| 3     | Learning profiles (skill tracker + mastery)             |
| 4     | Prompt intelligence (memories + profile in system prompt) |
| 5     | Recommendation engine                                   |
| 6     | Structured outputs + tool calling                       |
| 7     | Qdrant, embeddings, document RAG, session continuity    |
